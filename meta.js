...
const validate = sus => {
  ...
  if(meta.hasOwnProperty('DIFFICULTY') && meta.DIFFICULTY.LEVEL === 4) req.splice(req.indexOf('PLAYLEVEL'),1)
...
const susToMetaArray = sus => {
  return sus.split('\n')
    ...
    .map(line => ([line.slice(0,line.indexOf(' ')).trim().toUpperCase(), line.slice(line.indexOf(' ')+1,line.length).trim()]))
    .filter(line => support_meta.indexOf(line[0]) >= 0) // 非対応メタデータ除外
    .map(line => { if(line[1].slice(0,1) == '"' && line[1].slice(line[1].length-1) == '"') line[1] = line[1].slice(1,line[1].length-1); return line}) // 文字列リテラルの
    .filter(line => line[1])
    .map(line => {
      let data = line[1]
      switch(line[0]) {
